...
public class BookquantityView {
	...
	String account;
	public BookquantityView(String accout) {
		this.account=accout;
		initialize();
	}
				...
				if(borrowbook.equals("")){	
                    ...
                    return;
					 ...
					 try{
						 ...
						 String sqlCheckBookname = "SELECT * from BookInfo  WHERE Title = ?";  
						 try (PreparedStatement pstmtbookcheck = conn9.prepareStatement(sqlCheckBookname)) {  
							 ...
							 if (!rs2.next()) {  
								 JOptionPane.showMessageDialog(frame, "没有此书名书籍");  
								 return;  
							 }  
							 else{
								 panel_1.removeAll();
								 BookquantityPage pageBU1=new BookquantityPage(account,panel_2,panel_1,searchbook);
								 SwingUtilities.updateComponentTreeUI(panel_2);
								 ...
								 while (rs2.next()) {  
									 ...
									 BookquantityStock text=new BookquantityStock(BookID,Bookname,author,category,Brrownumber,panel_1,searchbook);
									 SwingUtilities.updateComponentTreeUI(panel_1);								
									 }
